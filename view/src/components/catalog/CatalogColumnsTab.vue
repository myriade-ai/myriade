<template>
  ...
  >
    <div
      ...
      :key="column.asset.id"
      :class="[
        'p-4 transition-colors space-y-3 cursor-pointer',
      ...
      <div class="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
        <div class="flex-1 min-w-0 space-y-2">
          ...
          <div class="space-y-1">
            ...
            <Badge variant="secondary">{{ column.privacyLabel }}</Badge>
          </div>
        </div>
        <div class="flex flex-col items-start gap-1 text-sm text-muted-foreground sm:items-end">
          <span>Completeness: {{ column.score }}%</span>
        </div>
      </div>
    </div>
  </div>
