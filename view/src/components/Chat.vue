<template>
  <div>
    ...
    <div
      ref="scrollContainer"
      class="flex justify-center px-2 sm:px-4 lg:px-0 min-h-[calc(100vh-4rem)]"
    ...
    >
      <div class="flex flex-col w-full lg:min-h-[calc(100vh-4rem)]">
          ...
          <div class="w-full lg:pt-4">
            <ul class="list-none">
              <template v-for="(group, index) in messageGroups" :key="index">
                <li v-for="message in group.publicMessages" :key="message.id" :class="message.role === 'user' ? 'flex justify-end' : ''">
                  <MessageDisplay
                    :message="message"
                ...
                >
                  ...
                  >
                    <MessageDisplay
                      ...
                      @editInlineClick="editInline"
                      @regenerateFromMessage="conversationsStore.regenerateFromMessage"
                      class="border-l-4 border-gray-300 pl-3 bg-gray-50/50"
                    />
                  </li>
                </transition-group>
              </template>
        ...
        <div
          id="chat-input"
          class="w-full max-w-3xl border-gray-300 sticky bottom-2 z-10 mx-auto lg:px-2 px-0"
        >
          <transition
            enter-active-class="transition-all duration-300 ease-out"
          ...
          <div
            id="input-container"
            class="bg-white border border-gray-300 rounded-lg shadow-lg p-3 sm:p-4"
          >
            <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-2 gap-2">
              <div class="flex space-x-2">
                ...
                <Button
                  @click="editMode = 'SQL'"
                  :variant="editMode == 'SQL' ? 'default' : 'secondary'"
                  size="sm"
                >
                  Editor
                </Button>
              ...
              <div
                ...
                class="text-sm text-gray-500 text-center sm:text-right"
              >
            ...
            <div class="w-full flex py-1 relative" v-if="editMode == 'SQL'">
              ...
              <div
                ...
                class="ml-2 flex-shrink-0 text-gray-300 hover:text-white"
              >
            ...
            <div class="w-full flex py-1 relative" v-else>
              <Textarea
                ...
                v-model="inputText"
                class="bg-transparent flex-1 resize-none"
              />
              <transition
              ...
              >
                <div v-if="inputText.trim().length > 0" class="ml-2 flex-shrink-0">
                  <SendButtonWithStatus :status="sendStatus" @clicked="handleSendMessage" />
                </div>
              </transition>
            </div>
...
/** HANDLE EVENTS **/
const inputTextarea = ref<any>(null)
const inputText = ref('')
const inputSQL = ref('')
...
const resizeTextarea = () => {
  ...
  nextTick(() => {
    ...
    // Access the underlying textarea element from the Vue component
    const textarea = inputTextarea.value.$el || inputTextarea.value
    if (textarea) {
