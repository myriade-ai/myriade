...
// Server-side search for fuzzy matching (triggered for queries >= 3 chars or when tag/status filters are selected)
...
// Convert search result IDs to a Set for O(1) lookup
// Use server results whenever search is enabled (text >= 3 OR tag/status filters active)
...
const filteredTree = computed(() =>
  buildFilteredTree({
    searchQuery: matchingIds.value ? '' : searchQuery.value, // Skip client-side text search if using server results
