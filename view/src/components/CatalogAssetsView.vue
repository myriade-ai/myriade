...
const columnsForSelectedTable = computed(() => {
  ...
  return tableColumns.map((column) => ({
    ...
    privacyLabel: column.column_facet?.privacy?.llm || 'Default',
...
const assetHasChanges = computed(() => {
  ...
  // Check privacy for columns
  if (asset.type === 'COLUMN') {
    const currentPrivacy = asset.column_facet?.privacy?.llm || 'Default'
    if (currentPrivacy !== assetDraft.privacy) {
      return true
    }
  }
...
watch(
  ...
  (asset) => {
    ...
    assetDraft.privacy = asset.column_facet?.privacy?.llm || 'Default'
    assetEditing.value = false
  },
  { immediate: true }
)
...
function updateAssetDraft(draft: EditableDraft) {
  ...
  assetDraft.privacy = draft.privacy
}
...
async function saveAssetDetails() {
  ...
  try {
    ...
    // Include privacy for columns
    if (asset.type === 'COLUMN') {
      updatePayload.privacy = assetDraft.privacy
    }
    ...
    assetDraft.privacy = updated.column_facet?.privacy?.llm || assetDraft.privacy
    assetEditing.value = false
  } catch (error) {
