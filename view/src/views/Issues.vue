  ...
  <div class="flex-1 overflow-auto">
    <div class="px-4 pt-4">
      ...
      <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div class="relative flex-grow max-w-md">
          ...
          <input
            ...
            class="block w-full pl-10 pr-4 py-2 border border-input rounded-md shadow-sm focus:border-primary-500 focus:outline-none focus:ring-primary-500 sm:text-sm bg-background text-foreground"
          />
        </div>
        <div class="flex items-center space-x-2 text-sm text-muted-foreground">
          <span
            >{{ filteredIssues.length }}
            {{ filteredIssues.length === 1 ? 'issue' : 'issues' }}</span
          >
          <button
            v-if="searchQuery"
            @click="searchQuery = ''"
      ...
      <div class="mb-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
        <!-- Entity Filter -->
        <div>
          <label for="entity-filter" class="block text-sm font-medium text-foreground mb-1"
            >Entity</label
          >
          <select
            id="entity-filter"
            v-model="selectedEntity"
            class="block w-full pl-3 pr-10 py-2 text-base border-input focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm rounded-md shadow-sm bg-background text-foreground"
          >
        ...
        <!-- Scope Filter -->
        <div>
          <label for="scope-filter" class="block text-sm font-medium text-foreground mb-1"
            >Scope</label
          >
          <select
            id="scope-filter"
        ...
        <div>
          <button
            @click="clearFilters"
            class="w-full sm:w-auto inline-flex items-center justify-center px-4 py-2 border border-border rounded-md shadow-sm text-sm font-medium text-foreground bg-card hover:bg-muted focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500"
          >
            Clear Filters
          </button>
        </div>
      ...
      <div v-if="store.loadingIssues" class="flex justify-center items-center py-10">
        ...
        <p class="ml-3 text-muted-foreground">Loading issues...</p>
      </div>
      ...
      <div v-else-if="filteredIssues.length > 0" class="space-y-4">
        ...
        >
            ...
            <div class="flex flex-col space-y-2 sm:hidden">
              <div class="flex items-center justify-between">
                <div class="flex items-center space-x-3">
                  ...
                  ></div>
                  <h3 class="text-base font-medium text-foreground line-clamp-1">
                    {{ issue.title }}
                  </h3>
                </div>
                <svg
                  class="h-5 w-5 text-muted-foreground transform transition-transform duration-200 flex-shrink-0"
              ...
              <div class="flex flex-wrap gap-2 text-xs">
                <span class="bg-muted text-muted-foreground font-medium px-2 py-1 rounded">
            ...
            <div class="hidden sm:flex sm:justify-between sm:items-center">
              <div class="flex items-center space-x-3">
                <div
                  ...
                  class="w-5 h-5 rounded-full flex-shrink-0"
                ></div>
                <h3 class="text-lg font-medium text-foreground">{{ issue.title }}</h3>
              </div>
              <div class="flex items-center">
                <!-- Entity Badge -->
                <span
                  class="bg-muted text-muted-foreground text-xs font-medium mr-4 px-2.5 py-1 rounded"
                >
                  {{ getEntityName(issue.business_entity_id) }}
                </span>
                <!-- Status Badge -->
                ...
                <svg
                  class="h-5 w-5 text-muted-foreground transform transition-transform duration-200"
                  :class="{ 'rotate-180': expandedIssue?.id === issue.id }"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
          ...
          <div v-if="expandedIssue?.id === issue.id" class="border-t border-border">
            <!-- View Mode -->
            <div v-if="expandedIssue.mode === 'view'" class="p-4 sm:p-5">
              <div class="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-2">
                <h4 class="text-base font-medium text-foreground">Issue Details</h4>
                <button
                  @click.stop="switchToEditMode(issue)"
                  class="inline-flex items-center text-sm px-3 py-1.5 border border-border rounded-md text-primary-700 hover:bg-primary-50 dark:hover:bg-primary-900/20 focus:outline-none focus:ring-2 focus:ring-primary-500"
                >
              ...
              <div class="sm:hidden space-y-4">
                <div class="bg-muted p-4 rounded-md">
                  <p class="text-xs uppercase tracking-wide font-medium text-muted-foreground mb-2">
                    Description
                  </p>
                  <MarkdownDisplay
                ...
                <div class="grid grid-cols-2 gap-3">
                  <div class="bg-muted p-3 rounded-md">
                    <p
                      class="text-xs uppercase tracking-wide font-medium text-muted-foreground mb-1"
                    >
                      Severity
                    </p>
                    <p class="text-sm text-foreground flex items-center">
                      <span
                ...
                <div class="grid grid-cols-1 gap-3">
                  ...
                  <div class="bg-muted p-3 rounded-md">
                    ...
                    <p class="text-sm text-foreground">{{ issue.scope }}</p>
                  </div>
                  ...
                  <div class="bg-muted p-3 rounded-md">
                    ...
                    <p class="text-sm text-foreground">{{ issue.database_id }}</p>
                  </div>
                  ...
                  <div class="bg-muted p-3 rounded-md">
                    ...
                    <p class="text-sm text-foreground">{{ issue.message_id }}</p>
                  </div>
                </div>
              </div>
              ...
              <div class="hidden sm:grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2">
                  <div class="bg-muted p-4 rounded-md h-full">
                    <p
                      class="text-xs uppercase tracking-wide font-medium text-muted-foreground mb-2"
                    >
                      Description
                    </p>
                    <MarkdownDisplay
              ...
              <form @submit.prevent="saveIssueChanges" class="space-y-6">
                <div>
                  ...
                  <input
                    ...
                    class="block w-full rounded-md border-input shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm p-2 bg-background text-foreground"
                ...
                <div class="sm:hidden space-y-4">
                  <div>
                    ...
                    <textarea
                      ...
                      rows="6"
                      class="block w-full rounded-md border-border bg-background shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm p-2"
                ...
                <div
                  class="flex flex-col sm:flex-row justify-end space-y-2 sm:space-y-0 sm:space-x-3 pt-4 border-t border-border"
                >
                  <button
                    type="button"
                    @click.stop="cancelEditMode"
                    class="px-4 py-2 border border-border rounded-md text-sm font-medium text-foreground bg-card hover:bg-muted focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 w-full sm:w-auto"
                  >
                    Cancel
                  </button>
                  <button
      ...
      <div
        ...
        class="text-center py-10 bg-card shadow-md rounded-lg border border-border"
      >
        <svg
        ...
        <h3 class="mt-2 text-sm font-medium text-foreground">No matching issues</h3>
      ...
      <div v-else class="text-center py-10 bg-muted shadow-md rounded-lg border border-border">
        <svg
          class="mx-auto h-12 w-12 text-muted-foreground"
          fill="none"
          viewBox="0 0 24 24"
        ...
        <h3 class="mt-2 text-sm font-medium text-foreground">No Issues Found</h3>
        <p class="mt-1 text-sm text-muted-foreground">There are currently no issues to display.</p>
      </div>
    </div>
  </div>
</template>
...
const getStatusTextClass = (status: string) => {
  ...
  if (s === 'CLOSED' || s === 'DONE') return 'text-muted-foreground'
