...
@api.route("/databases/<uuid:database_id>/catalog/dashboard-stats", methods=["GET"])
@user_middleware
def get_catalog_dashboard_stats(database_id: UUID):
    """
    Get aggregated dashboard statistics for a catalog database.
    Returns counts by status, completion percentage, and per-database/schema breakdowns.
    """
    database, error_response = _get_catalog_database(database_id)
    if error_response:
        return error_response
    ...
    # Query 1: Overall stats by asset type and status
    overall_stats = (
        g.session.query(
            Asset.type,
            Asset.status,
    ...
    for asset_type, status, count in overall_stats:
        ...
        if asset_type != "COLUMN":
            ...
            elif status in ("needs_review", "requires_validation"):
                assets_to_review += count

    # Query 2: Assets with descriptions (by type, exclude COLUMN)
    assets_with_desc = (
        g.session.query(
            Asset.type,
            func.count(Asset.id).label("count"),
    ...
    for asset_type, count in assets_with_desc:
        assets_with_description += count
        if asset_type in assets_by_type:
            assets_by_type[asset_type]["with_description"] = count

    # Calculate overall completion percentage
    non_column_total = sum(
        counts["total"]
        for asset_type, counts in assets_by_type.items()
        if asset_type != "COLUMN"
    )
    completion_percentage = (
        round((assets_with_description / non_column_total * 100), 1)
        if non_column_total > 0
        else 0
    )

    # Query 3: Get all DATABASE assets with their facets
    database_assets = (
        g.session.query(Asset.id, DatabaseFacet.database_name)
        .join(DatabaseFacet, DatabaseFacet.asset_id == Asset.id)
        .filter(Asset.database_id == database.id, Asset.type == "DATABASE")
        .all()
    )
        ...
        return jsonify(
            {
                "overall": {
                    ...
                    "assets_ai_generated": assets_ai_generated,
    ...
    database_asset_ids = [db_id for db_id, _ in database_assets]

    # Query 4: Bulk fetch all schemas for all databases
    all_schemas = (
        g.session.query(
            SchemaFacet.parent_database_asset_id,
            Asset.id,
            Asset.name,
            SchemaFacet.schema_name,
        )
        .join(Asset, Asset.id == SchemaFacet.asset_id)
        .filter(
            SchemaFacet.parent_database_asset_id.in_(database_asset_ids),
            Asset.type == "SCHEMA",
        )
        .all()
    )
    ...
    schemas_by_db = {}
    schema_ids_by_db = {}
    for parent_db_id, schema_id, schema_name, schema_facet_name in all_schemas:
        if parent_db_id not in schemas_by_db:
            schemas_by_db[parent_db_id] = []
            schema_ids_by_db[parent_db_id] = []
        schemas_by_db[parent_db_id].append(
            {
                "id": schema_id,
                "name": schema_name,
                "facet_name": schema_facet_name,
            }
        )
        schema_ids_by_db[parent_db_id].append(schema_id)

    # Get all schema IDs for bulk queries
    all_schema_ids = [
        schema_id for ids in schema_ids_by_db.values() for schema_id in ids
    ]

    if not all_schema_ids:
        # No schemas at all, return empty databases
        databases_data = [
            {
            ...
            for db_id, db_name in database_assets
        ]
        return jsonify(
            {
                "overall": {
    ...
    # Query 5: Bulk fetch table counts and descriptions per schema
    TableAsset = aliased(Asset)
    schema_table_stats = (
        g.session.query(
            TableFacet.parent_schema_asset_id,
            func.count(TableFacet.asset_id).label("table_count"),
            func.count(case((TableAsset.description.isnot(None), 1), else_=None)).label(
                "tables_with_desc"
            ),
        )
        .join(TableAsset, TableAsset.id == TableFacet.asset_id)
        .filter(
            TableFacet.parent_schema_asset_id.in_(all_schema_ids),
            TableAsset.type == "TABLE",
        )
        .group_by(TableFacet.parent_schema_asset_id)
        .all()
    )

    # Build lookup dict for schema stats
    schema_stats_lookup = {
        schema_id: {"table_count": tbl_count, "tables_with_desc": tbls_with_desc}
        for schema_id, tbl_count, tbls_with_desc in schema_table_stats
    }

    # Query 6: Bulk fetch table IDs per schema for column counts
    table_ids_query = (
        g.session.query(
            TableFacet.parent_schema_asset_id,
            Asset.id,
        )
        .join(Asset, Asset.id == TableFacet.asset_id)
        .filter(
            TableFacet.parent_schema_asset_id.in_(all_schema_ids),
            Asset.type == "TABLE",
        )
    ...
    table_ids_by_schema = {}
    for schema_id, table_id in table_ids_query:
        if schema_id not in table_ids_by_schema:
            table_ids_by_schema[schema_id] = []
        table_ids_by_schema[schema_id].append(table_id)

    # Get all table IDs for column count query
    all_table_ids = [
        table_id for ids in table_ids_by_schema.values() for table_id in ids
    ]

    # Query 7: Bulk fetch column counts per table
    column_counts = {}
    if all_table_ids:
        column_count_query = (
            g.session.query(
                ColumnFacet.parent_table_asset_id,
                func.count(Asset.id).label("column_count"),
            )
            .join(Asset, Asset.id == ColumnFacet.asset_id)
            .filter(
                ColumnFacet.parent_table_asset_id.in_(all_table_ids),
                Asset.type == "COLUMN",
            )
            .group_by(ColumnFacet.parent_table_asset_id)
            .all()
        )
        column_counts = {table_id: count for table_id, count in column_count_query}

    # Build response data structures
    databases_data = []
    for db_asset_id, db_facet_name in database_assets:
        schema_ids = schema_ids_by_db.get(db_asset_id, [])
        schemas_info = schemas_by_db.get(db_asset_id, [])

        if not schema_ids:
            databases_data.append(
                {
                    "database_id": str(db_asset_id),
                    "database_name": db_facet_name,
                    "total_schemas": 0,
                    "total_tables": 0,
                    "total_columns": 0,
                    "completion_percentage": 0,
        ...
        # Aggregate counts for this database
        total_tables = 0
        total_tables_with_desc = 0
        total_columns = 0
        schemas_data = []

        for schema_info in schemas_info:
            schema_id = schema_info["id"]
            stats = schema_stats_lookup.get(
                schema_id, {"table_count": 0, "tables_with_desc": 0}
            )
            tbl_count = stats["table_count"]
            tbls_with_desc = stats["tables_with_desc"]

            total_tables += tbl_count
            total_tables_with_desc += tbls_with_desc

            # Sum column counts for this schema's tables
            schema_table_ids = table_ids_by_schema.get(schema_id, [])
            schema_columns = sum(column_counts.get(tid, 0) for tid in schema_table_ids)
            total_columns += schema_columns

            schema_completion = (
                round((tbls_with_desc / tbl_count * 100), 1) if tbl_count > 0 else 0
            )
            schemas_data.append(
                {
                    "schema_name": schema_info["facet_name"] or schema_info["name"],
                    "schema_asset_id": str(schema_id),
        ...
        db_completion = (
            round((total_tables_with_desc / total_tables * 100), 1)
            if total_tables > 0
            else 0
        )
        ...
        databases_data.append(
            {
                ...
                "last_updated": None,
                "schemas": sorted(schemas_data, key=lambda x: x["schema_name"]),
            }
        )
