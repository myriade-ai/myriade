...
def sync_database_metadata_to_assets(
    ...
    for asset in orphaned_assets:
        if asset.type == "COLUMN":
            ...
            column_facets_to_delete = db_session.query(ColumnFacet).filter(
            ...
            for column_facet in column_facets_to_delete:
                column_asset = db_session.query(Asset).filter(
                    Asset.id == column_facet.asset_id
                ).first()
                if column_asset:
            ...
            table_facets_to_delete = db_session.query(TableFacet).filter(
            ...
            for table_facet in table_facets_to_delete:
                table_asset = db_session.query(Asset).filter(
                    Asset.id == table_facet.asset_id
                ).first()
                if table_asset:
                    # Delete column assets that reference this table
                    column_facets = db_session.query(ColumnFacet).filter(
            ...
            schema_facets_to_delete = db_session.query(SchemaFacet).filter(
            ...
            for schema_facet in schema_facets_to_delete:
                schema_asset = db_session.query(Asset).filter(
                    Asset.id == schema_facet.asset_id
                ).first()
                if schema_asset:
                    # Delete table assets that reference this schema
                    table_facets = db_session.query(TableFacet).filter(
                        TableFacet.parent_schema_asset_id == schema_asset.id
                    ).all()
